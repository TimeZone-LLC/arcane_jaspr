...
- Slider `onChanged` and `onRangeChanged` callbacks receive the values the
  interaction runtime reports through its `arcane:slider-change` event. The
  callbacks were never bound, so hydrated sliders moved without notifying the
  application in any theme.
- Neon sliders render their track, fill and thumb; the renderer emitted no
  geometry and the theme has no slider rules.
